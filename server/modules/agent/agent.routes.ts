...
export function createAgentRouter(dependencies: AgentRouterDependencies): express.Router {
  ...
  function validateBranchName(branchName) {
    ...
    const invalidPatterns = [
      // A leading dash is parsed as a flag (e.g. "-f" forces checkout/push)
      { pattern: /^-/, message: 'Branch name cannot start with a dash' },
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        try {
          ...
          // Only a missing directory (ENOENT) may proceed to clone. Any other
          // failure — including an existing directory held by a different or
          // invalid repository — must reach the caller instead of being
          // mistaken for "directory doesn't exist".
        ...
        const gitEnvironment = githubToken ? {
          ...
          GIT_CONFIG_VALUE_1: '!f() { echo username=x-access-token; echo "password=$DDAGENT_GITHUB_TOKEN"; }; f',
          DDAGENT_GITHUB_TOKEN: githubToken,
      ...
      if (createBranch || createPR) {
        try {
          ...
          if (createPR) {
            ...
            prBody += '\n\n---\n*This pull request was automatically created by ddagent.ai Agent.*';
