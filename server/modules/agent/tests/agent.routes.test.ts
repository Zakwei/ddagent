...
test('GitHub cloning keeps credentials out of arguments and remote URL', async () => {
  ...
  await withAgentServer(createDependencies({
    fileSystem: {
      // ENOENT is the only "directory missing" signal the clone path honors;
      // any other access error now aborts instead of proceeding to clone.
  ...
  assert.equal(cloneEnvironment?.DDAGENT_GITHUB_TOKEN, token);
