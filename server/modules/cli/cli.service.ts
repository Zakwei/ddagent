...
function showStatus(dependencies: CliServiceDependencies): void {
  ...
  output.log(`      ${terminalTextStyles.dim('>')} Use ${terminalTextStyles.bright('ddagent --port 8080')} to run on a custom port`);
  output.log(`      ${terminalTextStyles.dim('>')} Use ${terminalTextStyles.bright('ddagent --database-path /path/to/db')} for custom database`);
  output.log(`      ${terminalTextStyles.dim('>')} Run ${terminalTextStyles.bright('ddagent help')} for all options`);
...
Examples:
  $ ddagent                        # Start with defaults
  $ ddagent --port 8080            # Start on port 8080
  $ ddagent sandbox ~/my-project   # Run in a Docker sandbox
  $ ddagent status                 # Show configuration
...
Documentation:
  ${dependencies.packageMetadata.homepage || 'https://github.com/Zakwei/ddagent'}

Report Issues:
  ${dependencies.packageMetadata.bugsUrl || 'https://github.com/Zakwei/ddagent/issues'}
`);
}
...
export function createCliService(dependencies: CliServiceDependencies): CliApplication {
  const checkForUpdates = async (silent = false): Promise<boolean> => {
    try {
      ...
      if (isNewerVersion(latestVersion, currentVersion)) {
        ...
        dependencies.output.log(`         Run ${terminalTextStyles.bright('ddagent update')} to update\n`);
        return true;
      }
      if (!silent) {
  ...
  const updatePackage = async (): Promise<void> => {
    ...
    try {
      ...
      dependencies.output.log(`${terminalTextStyles.tip('[TIP]')} Try running manually: npm update -g @ddagent-ai/ddagent`);
    }
  };
    ...
    async run(argumentsList) {
      ...
      switch (parsedArguments.command) {
        ...
        default:
          ...
          dependencies.output.log('   Run "ddagent help" for usage information.\n');
          return 1;
      }
    },
  };
