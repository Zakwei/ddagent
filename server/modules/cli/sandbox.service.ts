...
const SANDBOX_TEMPLATES: Record<string, string> = {
  claude: 'docker.io/ddagentai/sandbox:claude-code',
  codex: 'docker.io/ddagentai/sandbox:codex',
};
...
Usage:
  ...
  ddagent sandbox <subcommand> [name]    Manage sandboxes
...
Examples:
  ...
  $ ddagent sandbox ~/my-project --agent codex --port 8080
  $ ddagent sandbox ~/my-project --env SERVER_PORT=8080 --env HOST=0.0.0.0
  $ ddagent sandbox ls
  $ ddagent sandbox stop my-project
  $ ddagent sandbox start my-project
  $ ddagent sandbox rm my-project
...
Advanced usage:
  ...
  or passing prompts to the agent, use sbx directly with the template:
    ...
    sbx run --template docker.io/ddagentai/sandbox:claude-code claude ~/project ~/libs:ro --memory 8g
...
function requireSandboxName(options: SandboxOptions, output: CliOutput): string | null {
  ...
  output.error(`\n${terminalTextStyles.error('❌')} Sandbox name required: ddagent sandbox ${options.subcommand} <name>\n`);
  return null;
}
    ...
    async execute(argumentsList) {
      ...
      switch (options.subcommand) {
        ...
        case 'logs': {
          ...
          try {
            dependencies.runSandboxCommand(
              ['exec', sandboxName, 'bash', '-c', 'cat /tmp/ddagent-ui.log'],
              true,
            );
          } catch (error) {
            dependencies.output.error(
        ...
        case 'start': {
          ...
          dependencies.runSandboxCommand([
            ...
            'bash',
            '-c',
            'nohup ddagent start --port 3001 > /tmp/ddagent-ui.log 2>&1 & disown',
          ]);
        ...
        case 'create': {
          if (!options.workspace) {
            dependencies.output.error(
              `\n${terminalTextStyles.error('❌')} Workspace path required: ddagent sandbox <path>\n`,
            );
            dependencies.output.log(`   Example: ${terminalTextStyles.bright('ddagent sandbox ~/my-project')}\n`);
            return 1;
          }
          ...
          dependencies.output.log(`\n${terminalTextStyles.dim('  Or install globally:')} npm install -g @ddagent-ai/ddagent\n`);
          return 0;
        }
