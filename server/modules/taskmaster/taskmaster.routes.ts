...
export function createTaskmasterRouter(dependencies: TaskmasterRouterDependencies): express.Router {
    ...
    router.get('/installation-status', async (req, res) => {
        try {
            // Tasks are stored and edited directly in .taskmaster/tasks/tasks.json
            // by this server, so the external task-master-ai CLI is no longer
            ...
            // MCP status stays informational only; it no longer gates the UI.
    ...
    router.post('/init/:projectId', async (req, res) => {
        try {
            ...
            // Create the .taskmaster directory natively; the external CLI is no
            // longer required and must not gate initialization.
            await taskmasterService.initializeProject(projectPath);
    ...
    router.post('/add-task/:projectId', async (req, res) => {
        try {
            const { projectId } = req.params;
            const { title, description, priority = 'medium', dependencies, details, testStrategy } = req.body;

            // A free-form prompt is no longer interpreted by an AI here; require
            // explicit fields so the task can be persisted verbatim.
            ...
            const task = await taskmasterService.addTask(projectPath, {
                title: String(title),
            ...
            // Broadcast task update via WebSocket using the projectId so clients
            // subscribed to this project get notified immediately.
            if (req.app.locals.wss) {
    ...
    router.put('/update-task/:projectId/:taskId', async (req, res) => {
        try {
            ...
            const updatedTask = await taskmasterService.updateTask(projectPath, taskId, {
                title,
                description,
                status,
                priority,
    ...
    router.post('/parse-prd/:projectId', async (req, res) => {
        try {
            ...
            // AI-backed PRD parsing needs an LLM provider, which the native
            // task store intentionally does not bundle. Manual task management
            // works without it; report the limitation instead of failing opaquely.
            return res.status(501).json({
                error: 'PRD parsing unavailable',
                message: 'Automatic PRD-to-task generation requires an AI provider. Add tasks manually from the board instead.',
            });
