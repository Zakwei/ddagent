...
`;
  ...
  "/cost": async (args, context) => {
    ...
    // Cache reads/writes are reported separately from fresh input so a
    // cache-heavy session is not mistaken for a huge prompt.
    const computedUsed = inputTokens + outputTokens + cacheReadTokens + cacheCreationTokens;
    const hasTokenBreakdown = computedUsed > 0;
    const used = Math.max(reportedUsed, computedUsed);
    const costUsd = Number(tokenUsage.costUsd ?? tokenUsage.cost_usd ?? tokenUsage.cost ?? 0) || 0;
...
router.post("/execute", async (req, res) => {
  try {
    ...
    // Handle built-in commands — own-property lookup so names like
    // "toString" or "constructor" cannot resolve functions inherited
    // from Object.prototype and run as handlers.
    const handler = Object.hasOwn(builtInHandlers, commandName)
    ...
    // Replace $ARGUMENTS with all arguments joined. The replacer function
    // keeps argument text literal: a string replacement would expand
    // patterns like $&, $', and $` inside user-supplied args.
