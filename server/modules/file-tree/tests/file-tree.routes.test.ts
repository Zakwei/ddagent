...
test('search route parses query options and forwards the project id', async () => {
  const inputs: Parameters<FileTreeServices['searchProjectFiles']>[] = [];
  const services = createFakeServices({
    searchProjectFiles: async (...input) => {
      inputs.push(input);
      return {
        results: [{ path: '/workspace/project/src/example.ts', line: 1, column: 0, text: 'hello' }],
        truncated: false,
      };
    },
  });
  ...
  await withFileTreeServer(services, async (baseUrl) => {
    const response = await fetch(
      `${baseUrl}/api/file-tree/projects/project-1/search?q=hello&respectGitignore=true&regex=true&limit=5`,
    );
  ...
  assert.deepEqual(inputs, [['project-1', 'hello', {
