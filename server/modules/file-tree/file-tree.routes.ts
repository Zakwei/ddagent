...
export function createFileTreeRouter(
  ...
  router.get('/projects/:projectId/search', createRouteHandler(async (request, response) => {
    const q = readOptionalString(request.query.q);
    if (!q || !q.trim()) {
      throw new AppError('Search query is required', { code: 'INVALID_SEARCH_QUERY', statusCode: 400 });
    }
    const respectGitignore = request.query.respectGitignore === 'true';
    const regex = request.query.regex === 'true';
    const limit = Number.parseInt(String(request.query.limit), 10);
    const result = await services.searchProjectFiles(
