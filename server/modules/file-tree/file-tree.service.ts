...
const HARD_EXCLUDED_DIRECTORY_NAMES = new Set([
  ...
  '.hypothesis', '.mcp', '.notebooklm', '.opencode', '.playwright',
  '.playwright-cli', '.project', '.taskmaster', '.venv-test', '.windsurf', '.zed',
]);
...
export function createFileTreeService(dependencies: FileTreeServiceDependencies): FileTreeServices {
  ...
  /**
   * Canonicalizes the nearest existing ancestor of `candidatePath` and
   * reattaches the segments that do not exist yet, so create and upload
   * targets can be checked before they are written. The filesystem root
   * always exists, so the walk terminates; non-ENOENT lookup errors
   * (for example EACCES) propagate to the caller.
   */
  async function canonicalizePath(candidatePath: string): Promise<string> {
    ...
    for (;;) {
      try {
        const realAncestor = await fileSystem.realpath(currentPath);
  ...
  /**
   * Extends `resolvePathInsideProject` with a canonical-path check so a
   * symlink inside the project cannot redirect reads or writes outside it.
   * The lexical resolved path is still returned because callers pass it to
   * the filesystem adapter; only the containment decision uses real paths.
   */
  async function resolveRealPathInsideProject(projectRoot: string, targetPath: string): Promise<string> {
    const resolvedPath = resolvePathInsideProject(projectRoot, targetPath);
    const realProjectRoot = await canonicalizePath(path.resolve(projectRoot));
    const realTargetPath = await canonicalizePath(resolvedPath);
    const normalizedRealRoot = realProjectRoot + path.sep;

    if (realTargetPath !== realProjectRoot && !realTargetPath.startsWith(normalizedRealRoot)) {
      throw createFileTreeError('Path must be under project root', 403, 'PATH_OUTSIDE_PROJECT');
    }
  ...
  return {
    ...
    async readTextFile(projectId, filePath) {
      ...
      const resolvedPath = await resolveRealPathInsideProject(projectRoot, filePath);
      try {
    ...
    async searchProjectFiles(projectId, query, options?) {
      const projectRoot = await resolveProjectRoot(projectId);
      const trimmedQuery = query.trim();
      if (!trimmedQuery || trimmedQuery.length > MAXIMUM_SEARCH_QUERY_LENGTH) {
        throw createFileTreeError('Search query is required', 400, 'INVALID_SEARCH_QUERY');
      }
      ...
      for (const directoryName of HARD_EXCLUDED_DIRECTORY_NAMES) {
        args.push('-g', `!**/${directoryName}/**`);
      }
      if (!options?.respectGitignore) {
        args.push('--no-ignore');
      }
      ...
      const searchProcess = spawn(RIPGREP_BINARY_PATH, args, {
        cwd: projectRoot,
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        function concludeSearch(): void {
          if (!processClosed || !readerClosed) {
            return;
          }
          if (killedByUs) {
            resolve();
          ...
          if (processError) {
            dependencies.logger.error('Failed to start ripgrep search', processError);
            reject(createFileTreeError('Search failed', 500, 'FILE_TREE_SEARCH_FAILED'));
            return;
          }
          if (exitCode === 1) {
            matches.length = 0;
          ...
          if (exitCode !== 0) {
            dependencies.logger.error(`ripgrep search failed with exit code ${exitCode}`);
            reject(createFileTreeError('Search failed', 500, 'FILE_TREE_SEARCH_FAILED'));
            return;
          }
          resolve();
        ...
        searchProcess.on('error', (error) => { processError = error; });
        searchProcess.on('close', (code) => {
          exitCode = code ?? -1;
          processClosed = true;
          concludeSearch();
        });
        searchProcess.stderr.on('data', () => {
          // Drain stderr so ripgrep never blocks on a full buffer.
        });
        ...
        lineReader.on('line', (line) => {
          if (killedByUs || !line.trim()) return;
          ...
          try {
            ...
            const rawText = typeof data.lines === 'object' && data.lines !== null
              ? String((data.lines as Record<string, unknown>).text ?? '')
              : '';
            let matchText = rawText.length > 200 ? rawText.slice(0, 200) : rawText;
            if (matchText.endsWith('\n')) {
              matchText = matchText.slice(0, -1);
            }

            matches.push({
              path: path.resolve(projectRoot, String((data.path as Record<string, unknown>)?.text ?? '')),
              line: Number(data.line_number ?? 0),
              column: Number(((data.submatches as unknown[] | undefined)?.[0] as Record<string, unknown> | undefined)?.start ?? 0),
              text: matchText,
            });
            ...
            // Ignore malformed JSON lines produced by ripgrep.
          }
        });
    ...
    async createEntry(input) {
      ...
      const targetPath = input.parentPath
        ...
        : input.name;
      const resolvedPath = await resolveRealPathInsideProject(projectRoot, targetPath);
    ...
    async renameEntry(input) {
      ...
      const projectRoot = await resolveProjectRoot(input.projectId);
      const resolvedOldPath = await resolveRealPathInsideProject(projectRoot, input.oldPath);
      ...
      await resolveRealPathInsideProject(projectRoot, resolvedNewPath);
      try {
    ...
    async deleteEntry(input) {
      const projectRoot = await resolveProjectRoot(input.projectId);
      const resolvedPath = await resolveRealPathInsideProject(projectRoot, input.targetPath);
      let stats;
      try {
    ...
    async storeUploadedFiles(input) {
      ...
      try {
        ...
        const resolvedTargetDirectory = !input.targetPath
          ...
          : await resolveRealPathInsideProject(projectRoot, input.targetPath);
        ...
        for (let fileIndex = 0; fileIndex < input.files.length; fileIndex += 1) {
          ...
          try {
            await resolveRealPathInsideProject(projectRoot, destinationPath);
          } catch (error) {
