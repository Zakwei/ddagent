...
/**
 * Stores chat image attachments in the global `~/.ddagent/assets` folder and
...
router.get('/images/:filename', async (req, res) => {
  ...
  asset.stream.on('error', (error) => {
    ...
    // Headers already went out, so no status can be sent — destroy the
    // socket to free the connection instead of letting the client hang
    // until timeout.
    res.destroy();
  });
});
