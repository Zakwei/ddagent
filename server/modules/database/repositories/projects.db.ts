...
export const projectsDb = {
    ...
    /**
     * FK-guard insert for background session indexers (`sessions.db`,
     * `devin-session-synchronizer.provider`): creates the row only when the
     * path is unknown and never flips `isArchived`, so indexing can't
     * resurrect a workspace the user removed.
     */
    ensureProjectPath(projectPath: string, customProjectName: string | null = null): void {
        const db = getConnection();
        const normalizedProjectPath = normalizeProjectPath(projectPath);
        const normalizedProjectName = normalizeProjectDisplayName(normalizedProjectPath, customProjectName);
        db.prepare(`
        INSERT OR IGNORE INTO projects (project_id, project_path, custom_project_name, isArchived)
            VALUES (?, ?, ?, 0)
