...
export const sessionsDb = {
   ...
   * from the first visible ddagent message by the sessions service.
   */
  createAppSession(
    sessionId: string,
    provider: string,
      ...
      .prepare(
        `SELECT ${SESSION_ROW_COLUMNS}
         ...
         WHERE provider = ?
           ...
           AND isArchived = 0
           AND ${SUBAGENT_SESSION_SQL_FILTER}
  ...
  /**
   * Removes indexed sessions by provider-native id or session id.
   *
   * Used by the synchronizers to drop child (subagent) sessions that an
   * earlier version indexed into the sidebar.
   */
  deleteProviderIndexedSessions(provider: string, providerSessionIds: string[]): number {
    ...
    for (let i = 0; i < providerSessionIds.length; i += CHUNK_SIZE) {
      const chunk = providerSessionIds.slice(i, i + CHUNK_SIZE);
