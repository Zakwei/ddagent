...
test('subagent sessions are filtered out from queries and paginated pages', async () => {
  await withIsolatedDatabase(() => {
    sessionsDb.createSession('session-main', 'opencode', '/workspace/demo-project', 'Main user session');
    sessionsDb.createSession('session-subagent-1', 'opencode', '/workspace/demo-project', 'Implement backend (@subagents/code/coder subagent)');
    sessionsDb.createSession('session-subagent-2', 'opencode', '/workspace/demo-project', 'Review backend (@subagents/code/reviewer subagent)');
    sessionsDb.createSession('session-subagent-3', 'opencode', '/workspace/demo-project', 'Explore codebase (@explore subagent)');
    sessionsDb.createSession('session-subagent-4', 'opencode', '/workspace/demo-project', 'General task (subagent)');

    const activeSessions = sessionsDb.getAllSessions();
    const projectSessions = sessionsDb.getSessionsByProjectPath('/workspace/demo-project');
    const paginatedSessions = sessionsDb.getSessionsByProjectPathPage('/workspace/demo-project', 10, 0);
    const count = sessionsDb.countSessionsByProjectPath('/workspace/demo-project');
    const recentPage = sessionsDb.getRecentSessionsPage(10, 0);

    assert.deepEqual(activeSessions.map((s) => s.session_id), ['session-main']);
    assert.deepEqual(projectSessions.map((s) => s.session_id), ['session-main']);
    assert.deepEqual(paginatedSessions.map((s) => s.session_id), ['session-main']);
