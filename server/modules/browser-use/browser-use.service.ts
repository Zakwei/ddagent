...
import { applyChromiumFontConfig, applyChromiumLibraryPath, getChromiumLaunchArgs, getModuleDirectory } from '@/shared/utils.js';

import { getBrowserUseRuntime } from './browser-use-runtime.js';

const require = createRequire(import.meta.url);
const __dirname = getModuleDirectory(import.meta.url);
const MAX_SESSIONS_PER_OWNER = Number.parseInt(process.env.DDAGENT_BROWSER_USE_MAX_SESSIONS_PER_OWNER || '3', 10);
const SESSION_TTL_MS = Number.parseInt(process.env.DDAGENT_BROWSER_USE_SESSION_TTL_MS || String(30 * 60 * 1000), 10);
...
const PROFILE_ROOT = path.join(os.homedir(), '.ddagent', 'browser-use', 'profiles');
const MCP_SERVER_NAME = 'ddagent-browser';
const LEGACY_MCP_SERVER_NAMES = ['ddagent-browser-use'];
...
const INSTALL_COMMAND_TIMEOUT_MS = Number.parseInt(
  process.env.DDAGENT_BROWSER_USE_INSTALL_TIMEOUT_MS || String(10 * 60 * 1000),
  10,
);
...
function formatInstallError(error: unknown): string {
  ...
  if (message.includes('sudo') && message.includes('password')) {
    return 'Installing Chromium system dependencies requires administrator privileges. Run `npx playwright install-deps chromium` on the machine where ddagent runs, then try again.';
  }
...
/**
 * Hosts the agent browser may never navigate to.
 *
 * The whatwg URL parser canonicalizes IPv4 literals (hex/octal/decimal and
 * short forms) into dotted-decimal and keeps brackets on IPv6, so prefix
 * checks on `parsed.hostname` cover every spelling. Loopback and link-local
 * are denied: the cloud metadata endpoint (169.254.169.254) sits on
 * link-local in every major cloud, and `browser_snapshot` returns page
 * content to the model, so reaching either would exfiltrate credentials or
 * probe internal services.
 */
function isDeniedNavigationHost(hostname: string): boolean {
  const host = hostname.toLowerCase();
  if (host === 'localhost' || host.endsWith('.localhost')) {
    return true;
  }
  if (host === '0.0.0.0' || host.startsWith('127.') || host.startsWith('169.254.')) {
    return true;
  }
  if (host === '[::1]' || /^\[fe[89ab]/.test(host)) {
    // [::1] is IPv6 loopback; fe80::/10 is IPv6 link-local.
    return true;
  }
  // IPv4-mapped IPv6 canonicalizes to "[::ffff:7f00:1]"; decode the embedded
  // address and apply the same IPv4 rules.
  const mapped = /^\[::ffff:([0-9a-f]{1,4}):([0-9a-f]{1,4})\]$/.exec(host);
  if (mapped) {
    const hi = Number.parseInt(mapped[1], 16);
    const lo = Number.parseInt(mapped[2], 16);
    const ipv4 = `${(hi >> 8) & 0xff}.${hi & 0xff}.${(lo >> 8) & 0xff}.${lo & 0xff}`;
    return ipv4 === '0.0.0.0' || ipv4.startsWith('127.') || ipv4.startsWith('169.254.');
  }
  return false;
}
...
function normalizeUrl(rawUrl: string): string {
  ...
  if (isDeniedNavigationHost(parsed.hostname)) {
    throw new Error('Navigation to loopback and link-local addresses is not allowed.');
  }
...
export const browserUseService = {
  ...
  async registerAgentMcp() {
    ...
    const results = await providerMcpService.addMcpServerToAllProviders({
      ...
      args,
      env: {
        DDAGENT_BROWSER_USE_MCP_TOKEN: getOrCreateMcpToken(),
        DDAGENT_BROWSER_USE_API_URL: getMcpApiUrl(),
      },
    });
  ...
  async createAgentSession(options?: { profileName?: string | null }) {
    ...
    let page: any;
    // Chromium inherits process.env at spawn; the vendored system libraries
    // must be on the loader path first or headless launches fail on containers
    // that cannot run `playwright install-deps`.
    ...
    // Any failure after launch must close what was opened: the browser is not
    // yet registered in `handles`, so nobody else would ever close it and the
    // Chromium process (and profile dir) would leak.
    try {
      if (profileName) {
