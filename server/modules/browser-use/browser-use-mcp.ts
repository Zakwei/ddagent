...
const apiUrl = (process.env.DDAGENT_BROWSER_USE_API_URL || 'http://127.0.0.1:3001/api/browser-use-mcp').replace(/\/$/, '');
const apiToken = process.env.DDAGENT_BROWSER_USE_MCP_TOKEN || '';
const API_TIMEOUT_MS = Number.parseInt(process.env.DDAGENT_BROWSER_USE_API_TIMEOUT_MS || '60000', 10);

async function callBrowserUseApi(toolName: string, input: Record<string, unknown>) {
  if (!apiToken) {
    throw new Error('DDAGENT_BROWSER_USE_MCP_TOKEN is not configured.');
  }
...
async function handleMessage(message: JsonRpcRequest) {
  if (message.method === 'initialize') {
    return {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: 'ddagent-browser', version: '1.0.0' },
    };
  }
