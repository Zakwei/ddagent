...
import { stageHunks, unstageHunks } from './git-hunk.service.js';
...
function spawnAsync(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    // `git apply` reads its patch from stdin; write it before the process exits.
...
function validateBranchName(branch) {
  // A leading dash turns the name into a git flag (e.g. "-f" makes
  // `git checkout -f` discard uncommitted work), so reject it separately.
...
function validateRemoteName(remote) {
  // Same leading-dash guard as branch names: "-f" would be parsed as
  // a force flag by git fetch/pull/push.
...
router.get('/status', async (req, res) => {
  ...
  try {
    ...
    res.status(isNotGitRepository ? 400 : 500).json({
...
router.get('/diff', async (req, res) => {
  ...
  try {
    ...
    const isNotGitRepository = error instanceof AppError && error.code === 'NOT_A_GIT_REPOSITORY';
    res.status(isNotGitRepository ? 400 : 500).json({ error: error.message, notGitRepository: isNotGitRepository });
  }
});
...
// Stage only the selected hunks of a file into the index. The diff is read
// server-side from git (never trusted from the client) so a caller cannot
// inject an arbitrary patch.
router.post('/stage-hunks', async (req, res) => {
  const { project, filePath, hunks } = req.body;

  if (!project || !filePath || !Array.isArray(hunks) || hunks.length === 0) {
    return res.status(400).json({ error: 'Project id, file path, and hunks are required' });
  }
  ...
  try {
    ...
    await validateGitRepository(projectPath);
    const repositoryRootPath = await getRepositoryRootPath(projectPath);
    const { repositoryRelativeFilePath } = await resolveRepositoryFilePath(projectPath, filePath);

    const { stdout: diff } = await spawnAsync(
      'git', ['diff', 'HEAD', '--', repositoryRelativeFilePath],
      { cwd: repositoryRootPath },
    );
    if (!diff.trim()) {
      return res.status(400).json({ error: 'No diff available for the selected file' });
    }
    ...
    console.error('Git stage hunks error:', error);
    res.status(500).json({ error: error.message });
  }
});

// Unstage only the selected hunks of a file, sourcing the patch from the index.
router.post('/unstage-hunks', async (req, res) => {
  ...
  try {
    ...
    const { stdout: diff } = await spawnAsync(
      'git', ['diff', '--cached', '--', repositoryRelativeFilePath],
      { cwd: repositoryRootPath },
    );
    if (!diff.trim()) {
      return res.status(400).json({ error: 'No staged diff available for the selected file' });
    }
    ...
    console.error('Git unstage hunks error:', error);
    res.status(500).json({ error: error.message });
  }
});
...
// Create a non-destructive working-tree snapshot (tracked + untracked) without
// touching the real index, so an AI turn can be undone afterwards.
...
router.get('/checkpoint/list', async (req, res) => {
  const { project } = req.query;
  ...
  try {
    ...
    const checkpoints = await listCheckpoints({ projectPath, runCommand: spawnAsync });

    res.json({ success: true, checkpoints });
  } catch (error) {
    console.error('Git checkpoint list error:', error);
    res.status(500).json({ error: error.message });
  }
});

// Restore the working tree to a checkpoint and remove untracked files the AI
// created after the snapshot.
router.post('/checkpoint/restore', async (req, res) => {
  ...
  if (!ref) {
    return res.status(400).json({ error: 'Checkpoint reference is required' });
  }
  ...
  try {
    ...
    const { removedUntracked } = await restoreCheckpoint({ projectPath, runCommand: spawnAsync, ref });

    res.json({
      success: true,
      removedUntracked,
      output: 'Working tree restored to the checkpoint.',
    });
  } catch (error) {
    console.error('Git checkpoint restore error:', error);
    res.status(500).json({ error: error.message });
  }
});
