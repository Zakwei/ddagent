...
/**
 * OpenCode managed skills install into its own user directory
 * (`~/.config/opencode/skills`) — never into the shared .claude/.agents
 * compatibility roots, which other providers own.
 */
test('providerSkillsService adds and removes managed global skills for opencode', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(createdSkill.scope, 'user');
    assert.equal(
      createdSkill.sourcePath.endsWith(
        path.join('.config', 'opencode', 'skills', 'opencode-global-dir', 'SKILL.md'),
      ),
      true,
    );
