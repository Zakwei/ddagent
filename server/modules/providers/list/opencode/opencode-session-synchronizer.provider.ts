...
export class OpenCodeSessionSynchronizer implements IProviderSessionSynchronizer {
  ...
  private synchronizeRows(since?: Date, limit?: number): SynchronizeRowsResult {
    ...
    try {
      const hasParentIdColumn = this.hasParentIdColumn(db);

      // OpenCode stores subagent (Task tool) sessions in the same `session`
      // table, linked through `parent_id` or with subagent markers in the title.
      // They are internal implementation details and must never surface as
      // top-level conversations, so delete any child rows and skip them below.
      this.removeIndexedChildSessions(db);
      ...
      const rows = db.prepare(`
        ...
        WHERE s.time_archived IS NULL
          ${hasParentIdColumn ? 'AND s.parent_id IS NULL' : ''}
  ...
  /**
   * Reports whether the attached OpenCode schema exposes the `parent_id`
   * column. Older releases lack it, so callers guard the column-specific SQL.
   */
  ...
  /**
   * Deletes sidebar rows that were indexed for OpenCode child (subagent)
   * sessions. They are internal execution details and must never surface in the UI.
   */
  private removeIndexedChildSessions(db: Database.Database): void {
    try {
      const hasParentId = this.hasParentIdColumn(db);
      const query = hasParentId
        ? `SELECT id, title FROM session WHERE parent_id IS NOT NULL OR title LIKE '%(@%subagent)%'`
        : `SELECT id, title FROM session WHERE title LIKE '%(@%subagent)%'`;
      const rows = db.prepare(query).all() as Array<{ id: unknown; title?: unknown }>;
      const childIds = rows
        .map((row) => readOptionalString(row.id))
      ...
      sessionsDb.deleteProviderIndexedSessions(this.provider, childIds);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.warn('[OpenCodeProvider] Failed to purge indexed subagent sessions:', message);
    }
  }
  ...
  private upsertSession(db: Database.Database, row: OpenCodeSessionRow): string | null {
    ...
    const titleCandidate = readOptionalString(row.title);
    if (isSubagentSessionTitle(titleCandidate)) {
      return null;
    }
    ...
    if (existingName && existingName !== fallbackTitle) {
      ...
      nextName = titleCandidate ?? this.readFirstUserText(db, sessionId);
    }
