...
/**
 * Maps one Anthropic streaming event from an SDK `stream_event` wrapper to the
 * live-only frames the client renders while the model is still generating.
 *
 * Only text and thinking deltas plus the end-of-message marker are forwarded.
 * `message_start`, `content_block_start`, `input_json_delta` and friends are
 * already covered by the complete `assistant` message, which stays the
 * authoritative copy and is emitted unchanged; returning an empty array keeps
 * those events off the websocket entirely.
 *
 * These frames are websocket-only. The JSONL transcript is written by the
 * Claude CLI itself from the complete messages, so nothing here reaches it.
 *
 * @param event - Raw stream event payload (`BetaRawMessageStreamEvent`)
 * @param sessionId - App-facing session id carried by the run
 * @returns Zero or one live frame for the client
 */
function normalizeClaudeStreamEvent(event: unknown, sessionId: string | null): NormalizedMessage[] {
  const streamEvent = readObjectRecord(event);
  if (!streamEvent) {
    return [];
  }
  ...
  if (streamEvent.type === 'content_block_delta') {
    ...
    if (delta?.type === 'thinking_delta' && typeof delta.thinking === 'string' && delta.thinking.length > 0) {
      return [createNormalizedMessage({ kind: 'thought_delta', content: delta.thinking, sessionId, provider: PROVIDER })];
    }
    return [];
  }

  // One `message_stop` per assistant message: the client closes the open live
  // text and thinking rows here, before the complete message arrives.
...
export class ClaudeSessionsProvider implements IProviderSessions {
  ...
  normalizeMessage(rawMessage: unknown, sessionId: string | null): NormalizedMessage[] {
    ...
    // Partial assistant messages arrive wrapped as `{ type: 'stream_event',
    // event }`; only the wrapper marks a live (never persisted) event, so the
    // unwrapping happens here rather than in the run loop.
    if (raw.type === 'stream_event') {
      // Subagent partials carry the grouping id of their parent tool call. The
      // client has no nested live transcript, so forwarding them would splice a
      // subagent's text into the main assistant row; their complete
      // tool_use/tool_result messages still flow through unchanged. Both the
      // raw SDK field and the run loop's camelCase copy are checked because the
      // adapter can be reached with either shape.
      if (raw.parentToolUseId || raw.parent_tool_use_id) {
        return [];
      }
      return normalizeClaudeStreamEvent(raw.event, sessionId);
    }
