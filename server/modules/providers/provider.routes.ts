...
const parseSessionWorkspacePath = (payload: unknown): string => {
  if (!payload || typeof payload !== 'object') {
    throw new AppError('Request body must be an object.', {
      code: 'INVALID_REQUEST_BODY',
      statusCode: 400,
  ...
  if (!projectPath) {
    throw new AppError('Workspace path is required.', {
      code: 'INVALID_WORKSPACE_PATH',
      statusCode: 400,
    });
  }

  if (projectPath.length > 1000) {
    throw new AppError('Workspace path must not exceed 1000 characters.', {
      code: 'INVALID_WORKSPACE_PATH',
      statusCode: 400,
    });
  }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const models = await providerModelsService.getProviderModels(provider, { forceRefresh });
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const sessionId = parseSessionId(req.params.sessionId);
    const projectPath = parseSessionWorkspacePath(req.body);
    const result = await sessionsService.updateSessionWorkspaceById(sessionId, projectPath);
    res.json(createApiSuccessResponse(result));
  }),
);
