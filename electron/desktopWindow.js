...
function isAllowedPermissionOrigin(sourceUrl, controlPlaneUrl) {
  try {
    ...
    return source.origin === controlPlane.origin || source.hostname.endsWith('.ddagent');
  } catch {
    return false;
  }
}
...
export class DesktopWindowManager {
  ...
  emitDesktopState() {
    ...
    if (this.mainWindow && !this.mainWindow.webContents.isDestroyed()) {
      this.mainWindow.webContents.send('ddagent-desktop:state-updated', state);
    }
    if (this.settingsWindow && !this.settingsWindow.webContents.isDestroyed()) {
      this.settingsWindow.webContents.send('ddagent-desktop:state-updated', state);
    }
  }

  emitLauncherCommand(command) {
    if (!this.mainWindow || this.mainWindow.webContents.isDestroyed()) return;
    this.mainWindow.webContents.send('ddagent-desktop:launcher-command', command);
  }

  emitSettingsCommand(command) {
    if (!this.settingsWindow || this.settingsWindow.webContents.isDestroyed()) return;
    this.settingsWindow.webContents.send('ddagent-desktop:launcher-command', command);
  }
      ...
      return [
        {
          ...
          click: () => void this.actions.connectCloudAccount()
            .catch((error) => this.actions.showError('Could not connect ddagent account', error)),
        },
      ];
    }
    ...
    const template = [
      ...
      },
      {
        ...
        submenu: [
          ...
          { type: 'separator' },
          {
            label: 'Open Local ddagent',
            accelerator: 'CmdOrCtrl+L',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local ddagent', error)),
          },
          {
        ...
        submenu: [
          {
            ...
            click: () => void this.actions.connectCloudAccount().catch((error) => this.actions.showError('Could not connect ddagent account', error)),
          },
          {
            label: 'Refresh Cloud Environments',
            click: () => void this.actions.refreshCloudEnvironments().catch((error) => this.actions.showError('Could not load ddagent environments', error)),
            enabled: Boolean(cloudState.account?.apiKey),
          },
          {
            label: 'Logout ddagent Account',
    ...
    const template = [
      {
        ...
        submenu: [
          {
            label: localState.localServerRunning ? 'Open Local in ddagent' : 'Start Local in ddagent',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local ddagent', error)),
          },
          {
  ...
  async createWindow() {
    ...
    // Guest webviews must never inherit our preload or Node access.
    this.mainWindow.webContents.on('will-attach-webview', (_event, webPreferences) => {
      try {
        ...
        this.actions.showError('Could not secure embedded webview', error);
      }
    });

    this.mainWindow.webContents.on('did-attach-webview', (_event, guestWebContents) => {
      if (!guestWebContents || guestWebContents.isDestroyed()) return;
      guestWebContents.setWindowOpenHandler(({ url }) => {
        if (/^https?:/i.test(url) && !guestWebContents.isDestroyed()) {
