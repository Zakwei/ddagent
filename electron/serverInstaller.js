...
/**
 * Installs the versioned local server runtime used by ddagent Desktop.
 *
 * Server bundles are cached under:
 *   ~/.ddagent/server/<version>/dist-server/server/index.js
 */

const DEFAULT_INSTALL_ROOT = path.join(os.homedir(), '.ddagent', 'server');
const DEFAULT_BUNDLE_BASE_URL = 'https://github.com/Zakwei/ddagent/releases/download';
...
export class ServerInstaller {
  constructor({
    ...
    arch = process.arch,
    installRoot = process.env.DDAGENT_SERVER_DIR || DEFAULT_INSTALL_ROOT,
    bundleBaseUrl = process.env.DDAGENT_SERVER_BUNDLE_URL || DEFAULT_BUNDLE_BASE_URL,
    bundleReleaseTag = process.env.DDAGENT_SERVER_BUNDLE_RELEASE_TAG || '',
    onLog,
  } = {}) {
  ...
  getBundleName() {
    return `ddagent-local-server-${this.version}-${this.platform}-${this.arch}.tar.gz`;
  }
