...
const SERVER_MARKER_PATH = path.join(os.homedir(), '.ddagent', 'local-server.json');
const LOCAL_SERVER_URL_ENV_KEYS = [
  'DDAGENT_DESKTOP_LOCAL_SERVER_URL',
  'DDAGENT_LOCAL_SERVER_URL',
  'ELECTRON_LOCAL_SERVER_URL',
];
const LOCAL_SERVER_PORT_ENV_KEYS = [
  'DDAGENT_DESKTOP_LOCAL_SERVER_PORT',
  'DDAGENT_SERVER_PORT',
  'SERVER_PORT',
  'PORT',
];
...
async function isDdagentServer(baseUrl) {
...
async function waitForDdagentServer(baseUrl, timeoutMs) {
  const startedAt = Date.now();

  while (Date.now() - startedAt < timeoutMs) {
    if (await isDdagentServer(baseUrl)) {
      return true;
    }
...
export class LocalServerController {
  ...
  async resolveServerEntry() {
    ...
    if (process.env.DDAGENT_USE_INSTALLED_SERVER !== '1' && await pathExists(bundledEntry)) {
      return bundledEntry;
    }
  ...
  startBundledServer(port, serverEntry) {
    ...
    this.ownedServerProcess.once('exit', (code, signal) => {
      ...
      if (this.ownedServerProcess) {
        console.error(`ddagent desktop server exited with code ${code ?? 'null'} and signal ${signal ?? 'null'}`);
      }
      this.ownedServerProcess = null;
    });
  }
  ...
  async resolveLocalServerUrl() {
    ...
    if (devUrl) {
      const ready = await waitForDdagentServer(defaultUrl, SERVER_START_TIMEOUT_MS);
      if (!ready) {
    ...
    if (!forceOwnServer) {
      ...
      for (const candidateUrl of candidateUrls) {
        if (await isDdagentServer(candidateUrl)) {
          const displayUrl = getDisplayUrl(candidateUrl);
          this.localServerPort = getPortFromUrl(candidateUrl);
          this.appendStartupLog(`Using existing Local ddagent at ${displayUrl}`);
          return displayUrl;
        }
      }
    }
    ...
    const ready = await waitForDdagentServer(serverUrl, SERVER_START_TIMEOUT_MS);
    if (!ready) {
    ...
    this.appendStartupLog(`Local ddagent ready at ${displayUrl}`);
