...
/**
 * Text rows the newest page should contain before the initial load stops
 * walking older pages. Tool-heavy turns push chat text out of a single page,
 * which used to leave the last assistant reply unfetched — and therefore
 * invisible — on every reload.
 */
export const INITIAL_HISTORY_MIN_TEXT_MESSAGES = 2;

/** Hard cap on the extra pages the initial load may fetch. */
export const INITIAL_HISTORY_MAX_EXTRA_PAGES = 3;

/**
 * Page size for the initial walk. The default 40-row page is routinely
 * swallowed whole by tool traffic, so the walk asks for a wider slice instead
 * of spending its page budget on 40-row steps.
 */
export const INITIAL_HISTORY_PAGE_SIZE = SESSION_MESSAGES_PAGE_SIZE * 5;

export function shouldFetchOlderInitialHistory(
  messages: NormalizedMessage[],
  hasMore: boolean,
  extraPagesFetched: number,
): boolean {
  if (!hasMore || extraPagesFetched >= INITIAL_HISTORY_MAX_EXTRA_PAGES) return false;

  let textRows = 0;
  for (const message of messages) {
    if (message.kind === 'text') {
      textRows += 1;
      if (textRows >= INITIAL_HISTORY_MIN_TEXT_MESSAGES) return false;
    }
  }
  return true;
}
