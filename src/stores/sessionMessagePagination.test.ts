...
import {
  INITIAL_HISTORY_MAX_EXTRA_PAGES,
...
test('initial history keeps walking while the tail has no chat text', () => {
  const toolOnlyPage = range(0, 39).map((row) => ({ ...row, kind: 'tool_result' as const }));

  assert.equal(shouldFetchOlderInitialHistory(toolOnlyPage, true, 0), true);
});

test('initial history stops once the window reaches the conversation', () => {
  const withText = [
    ...range(0, 38).map((row) => ({ ...row, kind: 'tool_result' as const })),
    message(39),
    message(40),
  ];

  assert.equal(shouldFetchOlderInitialHistory(withText, true, 0), false);
});

test('initial history respects the page budget and the end of the log', () => {
  const toolOnlyPage = range(0, 39).map((row) => ({ ...row, kind: 'tool_use' as const }));

  assert.equal(shouldFetchOlderInitialHistory(toolOnlyPage, true, INITIAL_HISTORY_MAX_EXTRA_PAGES), false);
  assert.equal(shouldFetchOlderInitialHistory(toolOnlyPage, false, 0), false);
});
