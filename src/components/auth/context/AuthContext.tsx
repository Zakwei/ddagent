...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  // The initial status check must run exactly once. `checkAuthStatus` changes
  // identity whenever `token` does (e.g. a mid-JWT-life refresh), and without
  // this guard the mount effect would re-run on every refresh — flipping
  // isLoading and remounting everything gated on it.
  const hasCheckedAuthStatusRef = useRef(false);
  ...
  useEffect(() => {
    ...
    if (hasCheckedAuthStatusRef.current) {
      return;
    }
    hasCheckedAuthStatusRef.current = true;
