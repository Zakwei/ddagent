...
Renders as a single `oc-tool-row` line (opencode-style icon glyph + label + value). Supports multiple rendering modes based on `action`:

- **open-file** — Value rendered as a clickable link that opens the file
- **jump-to-results** — Adds an anchor link to the result section
