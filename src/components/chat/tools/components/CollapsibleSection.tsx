...
export const CollapsibleSection: React.FC<CollapsibleSectionProps> = ({
  ...
  return (
    <Collapsible defaultOpen={open} className={cn('group/section', className)}>
      {/* When there's a clickable title (Edit/Write), the chevron and the tool
          name toggle collapse while the file path opens the file. */}
      {onTitleClick ? (
        <div className="flex cursor-default select-none items-center gap-1.5 py-0.5 text-xs group-data-[state=open]/section:sticky group-data-[state=open]/section:top-0 group-data-[state=open]/section:z-10 group-data-[state=open]/section:-mx-1 group-data-[state=open]/section:bg-background group-data-[state=open]/section:px-1">
          <CollapsibleTrigger className="flex flex-shrink-0 items-center gap-1.5 p-0.5 text-muted-foreground hover:text-foreground">
            <svg
          ...
          <button
            ...
            className="oc-tool-block-title flex-1 truncate text-left font-mono text-primary transition-colors hover:text-primary/80 hover:underline"
          >
            {title}
          </button>
