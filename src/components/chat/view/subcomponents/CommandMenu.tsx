...
const getMenuPosition = (position: NonNullable<CommandMenuProps['position']>): CSSProperties => {
  ...
  const boundsLeft = Math.max(MENU_EDGE_GAP, position.containerLeft ?? MENU_EDGE_GAP);
  const boundsRight = Math.min(
    window.innerWidth - MENU_EDGE_GAP,
    position.containerRight ?? window.innerWidth - MENU_EDGE_GAP,
  );
  // Never wider than the tile itself — a fixed floor would bleed onto the
  // neighboring pane in narrow splits.
  const menuWidth = Math.min(440, Math.max(0, boundsRight - boundsLeft));
  const clampedLeft = Math.max(
    boundsLeft,
    Math.min(position.left, boundsRight - menuWidth),
  );
...
export default function CommandMenu({
  ...
  if (commands.length === 0) {
    return renderInPortal(
      <div
        ref={menuRef}
        className="command-menu command-menu-empty oc-chat border border-border bg-popover/95 text-sm text-muted-foreground"
        style={{
          ...menuBaseStyle,
          ...menuPosition,
          overflowY: 'hidden',
  ...
  return renderInPortal(
    <div
      ...
      className="command-menu oc-chat overscroll-contain border border-border bg-popover/95 text-popover-foreground [-webkit-overflow-scrolling:touch]"
          ...
          {(groupedCommands[namespace] || []).map(({ command, commandIndex, renderKey }) => {
            ...
            return (
              <div
                ...
                className={`command-item group relative mb-1 flex min-h-[44px] cursor-pointer touch-manipulation items-center gap-2.5 rounded-md border px-2.5 py-2 transition-all ${
                  isSelected
              ...
              >
                ...
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md border ${accentClass}`}>
                  <NamespaceIcon aria-hidden="true" size={14} strokeWidth={2.2} />
                </span>
                <div className="min-w-0 flex-1 pr-1">
                  <div className={`flex min-w-0 items-center gap-2 ${command.description ? 'mb-0.5' : ''}`}>
                    <span
                    ...
                    {command.metadata?.type && (
                      <span className="command-metadata-badge inline-flex shrink-0 items-center rounded border border-border bg-muted px-1.5 py-0.5 text-[10px] font-medium leading-none text-muted-foreground shadow-sm">
                        {command.metadata.type}
                      </span>
                    )}
                  </div>
                ...
                {isSelected && (
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded border border-primary/30 bg-card text-primary shadow-sm">
