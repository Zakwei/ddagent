...
interface ComposerPermissionMenuProps {
  ...
  /** Optional "auto-continue tasks" toggle shown below the mode list. */
          ...
          {permissionModes.map((mode) => {
            ...
            return (
              <ComposerMenuItem
                ...
                // The generic `permissionModes.*` keys are provider-agnostic;
                // the legacy `codex.*` keys remain as the translated fallback
                // until per-provider catalogs exist.
                label={t([`permissionModes.modes.${mode}`, `codex.modes.${mode}`], { defaultValue: mode })}
                description={t([`permissionModes.descriptions.${mode}`, `codex.descriptions.${mode}`], { defaultValue: '' }) || undefined}
          ...
          {onToggleAutoContinueTasks && (
            <>
              ...
              <ComposerMenuItem
                ...
                label={t('input.autoContinueTasks', { defaultValue: 'Auto-continue' })}
                description={
                  t('input.autoContinueTasksTooltip', {
                    defaultValue: 'Enable to let Devin automatically continue to the next Task Master task',
                  }) || undefined
                }
                isSelected={Boolean(autoContinueTasks)}
                onSelect={onToggleAutoContinueTasks}
              />
            </>
          )}
        </ComposerMenuSurface>,
