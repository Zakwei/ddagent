...
import { getModelTier } from '../../hooks/useFavoriteModels';
...
function ModelsContent({
  ...
  const displayOptions = useMemo(() => {
    ...
    return filteredOptions.filter((option) => getModelTier(option) === tierFilter);
  }, [filteredOptions, tierFilter]);
  ...
  const showTierFilter = availableOptions.length > 1;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      ...
      {showTierFilter && (
        <PillBar>
          <Pill isActive={tierFilter === 'all'} onClick={() => setTierFilter('all')}>All</Pill>
          <Pill isActive={tierFilter === 'free'} onClick={() => setTierFilter('free')}>Free</Pill>
          <Pill isActive={tierFilter === 'paid'} onClick={() => setTierFilter('paid')}>Paid</Pill>
        </PillBar>
      )}
        ...
        <div className="scrollbar-thin -mr-1 min-h-0 flex-1 overflow-y-auto pr-1">
          <div className="grid gap-2 md:grid-cols-2">
            {displayOptions.map((option, index) => {
              ...
              const isFree = getModelTier(option) === 'free';
              return (
                <button
                  key={option.value}
                  type="button"
                ...
                >
                  <span className="flex items-center justify-between gap-2">
                    ...
                    <span className="flex shrink-0 items-center gap-2">
                      {isFree && (
                        <Badge className="rounded-full border-emerald-500/30 bg-emerald-500/15 px-2 py-0 text-[9px] font-medium text-emerald-700 hover:bg-emerald-500/20">
                          Free
                        </Badge>
                      )}
...
function CostContent({ data }: { data: CostCommandData }) {
  ...
  const cacheReadTokens = Number(data.tokenBreakdown?.cacheRead ?? 0);
  const cacheCreationTokens = Number(data.tokenBreakdown?.cacheCreation ?? 0);
  // Prefer the provider-reported amount when available; otherwise estimate.
  const reportedCost = Number(data.costUsd ?? 0);
  const estimatedCost = reportedCost > 0
    ? reportedCost
    : estimateCostUsd({
        model: data.model,
        inputTokens: Number(data.tokenBreakdown?.input ?? 0),
        outputTokens: Number(data.tokenBreakdown?.output ?? 0),
        cacheReadTokens,
        cacheCreationTokens,
      });
  const usageRows = [
            ...
            ? [
                {
                  ...
                  value: formatNumber(cacheReadTokens),
                  icon: TerminalSquare,
                },
              ]
            : []),
    ...
    ...(estimatedCost !== null
      ? [
          {
            label: reportedCost > 0 ? 'Cost' : 'Estimated cost',
...
function StatusContent({ data }: { data: StatusCommandData }) {
  ...
  const rows = [
    { label: 'Package', value: data.packageName || 'ddagent', icon: Package },
