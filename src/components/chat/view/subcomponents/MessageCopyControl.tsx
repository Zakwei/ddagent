...
const MessageCopyControl = ({
  ...
  // `contain: paint` box (which would otherwise clip it). Anchor its left edge
  // to the trigger, flipping above when there isn't room below. (Measuring
  // `right` from window.innerWidth pushed the menu off-screen in split view
  // when the trigger sat in a right-hand pane.)
  const openDropdown = () => {
    const rect = triggerRef.current?.getBoundingClientRect();
    if (rect) {
      const ESTIMATED_MENU_HEIGHT = 84;
      const ESTIMATED_MENU_WIDTH = 160; // min-w-36 (144px) + padding slack
      const openUp = rect.bottom + ESTIMATED_MENU_HEIGHT + 8 > window.innerHeight;
      setMenuStyle({
        position: 'fixed',
        left: Math.max(8, Math.min(rect.left, window.innerWidth - ESTIMATED_MENU_WIDTH - 8)),
        zIndex: 1000,
        ...(openUp
  ...
  useEffect(() => {
    ...
    // Move focus into the menu so Arrow/Escape handling works right away.
  ...
  const handleFormatChange = (format: CopyFormat) => {
    ...
    setIsDropdownOpen(false);
    // The menu unmounts on select — drop focus back on the trigger instead of
    // letting it fall to <body>.
    triggerRef.current?.focus();
  };

  // Roving-focus navigation between the menu items.
