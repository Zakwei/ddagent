...
/**
 * Author label for an assistant message: the provider recorded on the
 * message itself, so exports from non-Claude sessions are not mislabeled.
 */
function assistantAuthorLabel(msg: ChatMessage): string {
...
export function exportToMarkdown(
  ...
  for (const msg of messages) {
    if (msg.type === 'user') {
      ...
      markdown += `## ${assistantAuthorLabel(msg)}\n\n`;
...
export function exportToHTML(
  ...
  const htmlContent = messages
    .map((msg) => {
      const type = msg.type === 'user' ? '👤 You' : msg.type === 'assistant' ? `🤖 ${assistantAuthorLabel(msg)}` : `${msg.type}`;
