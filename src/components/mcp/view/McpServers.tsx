import { AlertTriangle, Edit3, ExternalLink, Globe, Lock, Plus, Server, Terminal, Trash2, Users, Zap } from 'lucide-react';
...
// Servers prefixed with `ddagent-` are written and removed automatically by a
// ddagent feature toggle (e.g. the Browser tab), not added by the user. They are
// shown read-only so users don't edit/delete them out of sync with the feature.
const isManagedServer = (server: ProviderMcpServer): boolean => server.name.startsWith('ddagent-');
    ...
    <div className="rounded-xl border border-dashed border-border/60 bg-muted/20 p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <a
            href="https://github.com/Zakwei/ddagent"
            target="_blank"
...
export default function McpServers({ selectedProvider, currentProjects }: McpServersProps) {
  ...
  const [pendingDeleteServer, setPendingDeleteServer] = useState<ProviderMcpServer | null>(null);
  ...
  const globalAddDescription = 'Add Global MCP Server writes one common stdio or HTTP server to Claude, Cursor, Codex, OpenCode, and Devin.';
  const providerAddDescription = `${providerButtonLabel} only changes ${providerName}.`;
  const globalModalDescription = 'Adds this MCP server to every provider: Claude, Cursor, Codex, OpenCode, and Devin. '
  ...
  return (
    <div className="space-y-4">
                    ...
                    {managed && (
                      <div className="text-xs text-muted-foreground">
                        {t('mcpServers.managed.hint', {
                          defaultValue: 'Managed by ddagent.',
                        })}
                      </div>
                    )}
                  </div>
      ...
      {/* Deleting a server rewrites provider config files, so it requires confirmation. */}
      <Dialog
        open={pendingDeleteServer !== null}
        onOpenChange={(nextOpen) => {
          if (!nextOpen) setPendingDeleteServer(null);
        }}
      >
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <DialogTitle className="not-sr-only text-base font-semibold">
                {t('mcpServers.deleteConfirm.title', { defaultValue: 'Delete MCP server?' })}
              </DialogTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                {t('mcpServers.deleteConfirm.description', {
                  serverName: pendingDeleteServer?.name ?? '',
                  defaultValue: '"{{serverName}}" will be removed from the provider configuration.',
                })}
              </p>
            </div>
          </div>
          <div className="mt-5 flex justify-end gap-2">
            <Button variant="ghost" size="sm" onClick={() => setPendingDeleteServer(null)}>
