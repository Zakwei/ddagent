...
export default function McpServerFormModal({
  ...
  return (
    ...
    // trap, Escape and backdrop-click dismissal, and autofocus on open. It is
    // stacked above the Settings dialog, which this modal opens on top of.
    <Dialog open={isOpen} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }}>
      <DialogContent
        wrapperClassName="z-[10000]"
        className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border-border bg-background p-0"
      >
        <div className="flex items-center justify-between border-b border-border p-4">
          <DialogTitle className="not-sr-only text-lg font-medium text-foreground">{modalTitle}</DialogTitle>
