...
export default function Shell({
  ...
  // Focus follows pointer: hovering the terminal gives it keyboard focus. No
  // `isActive` gate — focusing xterm activates the pane via focus capture in
  // SplitWorkspaceGrid, so gating would leave inactive panes unreachable.
  const handleTerminalPointerEnter = useCallback((event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    // Same commit-on-blur guard as ChatInterface: inputs and contentEditable
    // commit/close on blur, while pane-local textareas hand off harmlessly.
  ...
  }, [preferences.focusFollowsPointer, isInitialized, isConnected, terminalRef]);
  ...
  const handleCopyOutput = useCallback(async (): Promise<boolean> => {
    const term = terminalRef.current;
  ...
  const handleZoomIn = useCallback(() => {
    ...
    if (currentSize >= 24) return;
    const nextSize = Math.min(24, currentSize + 1);
  ...
  const handleZoomOut = useCallback(() => {
    ...
    const nextSize = Math.max(9, currentSize - 1);
    term.options.fontSize = nextSize;
    setFontSize(nextSize);
    const fit = fitAddonRef.current;
    if (fit) {
  ...
  return (
    <div className="flex h-full w-full flex-col bg-gray-900">
      <ShellHeader
        ...
        killLabel={t('shell.actions.kill', { defaultValue: 'Kill (SIGINT)' })}
        killTitle={t('shell.actions.killTitle', { defaultValue: 'Kill running process (Ctrl+C)' })}
        copyOutputLabel={t('shell.actions.copyOutput', { defaultValue: 'Copy output' })}
        copyOutputTitle={t('shell.actions.copyOutputTitle', { defaultValue: 'Copy terminal output' })}
        copiedLabel={t('shell.actions.copied', { defaultValue: 'Copied!' })}
        zoomInTitle={t('shell.actions.zoomInTitle', { defaultValue: 'Zoom in' })}
        zoomOutTitle={t('shell.actions.zoomOutTitle', { defaultValue: 'Zoom out' })}
      ...
      <div className="relative flex-1 overflow-hidden p-2">
        <div
          ...
          style={{ outline: 'none' }}
          onPointerEnter={handleTerminalPointerEnter}
        />
