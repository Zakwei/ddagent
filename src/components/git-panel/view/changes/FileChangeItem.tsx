...
import GitDiffViewer, { type GitDiffHunkAction, type GitDiffViewMode } from '../shared/GitDiffViewer';
...
type FileChangeItemProps = {
  ...
  // Optional per-hunk staging. Omitted in read-only usages (e.g. history), so
  // no hunk controls render there.
  hunkAction?: Omit<GitDiffHunkAction, 'onAction'> & { onAction: (filePath: string, hunkIndex: number) => void };
};
          ...
          {diff && (
            <GitDiffViewer
              ...
              hunkAction={hunkAction && {
                variant: hunkAction.variant,
                title: hunkAction.title,
                onAction: (hunkIndex) => hunkAction.onAction(filePath, hunkIndex),
              }}
            />
          )}
        </div>
