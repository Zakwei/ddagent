      ...
      >
        <div className="p-6">
          <h3 id="new-worktree-title" className="mb-1 text-lg font-semibold text-foreground">
            {t('gitPanel.newWorktree.title', 'New Worktree')}
          </h3>
          <p className="mb-4 text-sm text-muted-foreground">
            {t('gitPanel.newWorktree.description', 'Check out a branch in its own folder and work on it in parallel.')}
          </p>

          <div className="mb-4">
            <label htmlFor="worktree-branch-name" className="mb-2 block text-sm font-medium text-foreground/80">
              {t('gitPanel.newWorktree.branchLabel', 'Branch')}
            </label>
            <input
            ...
            {branchExists && (
              <p className="mt-1.5 text-xs text-muted-foreground">
                {t('gitPanel.newWorktree.existingBranch', 'Existing branch — it will be checked out as-is.')}
              </p>
            )}
          </div>
          ...
          {!branchExists && (
            <div className="mb-4">
              <label htmlFor="worktree-base-branch" className="mb-2 block text-sm font-medium text-foreground/80">
                {t('gitPanel.newWorktree.createFrom', 'Create from')}
              </label>
              <select
          ...
          {folderPreview && (
            <p className="mb-4 rounded-lg bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
              {t('gitPanel.newWorktree.willCreateIn', 'Will be created in')} <span className="font-mono text-foreground/80">{folderPreview}</span>
            </p>
          )}
          ...
          <label className="mb-4 flex cursor-pointer items-center gap-2 text-sm text-foreground/80">
            ...
            />
            {t('gitPanel.newWorktree.switchAfter', 'Switch to the worktree after creating it')}
          </label>
            ...
            >
              {isCreating ? (
                <>
                  ...
                  <span>{t('gitPanel.creating', 'Creating...')}</span>
                </>
              ) : (
                <>
                  <GitFork className="h-3 w-3" />
                  <span>{t('gitPanel.newWorktree.submit', 'Create Worktree')}</span>
                </>
              )}
            </button>
          </div>
