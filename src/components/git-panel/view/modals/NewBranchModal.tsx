...
export default function NewBranchModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="p-6">
          <h3 className="mb-4 text-lg font-semibold text-foreground">{t('gitPanel.newBranch.title', 'Create New Branch')}</h3>

          <div className="mb-4">
            <label htmlFor="git-new-branch-name" className="mb-2 block text-sm font-medium text-foreground/80">
              {t('gitPanel.newBranch.nameLabel', 'Branch Name')}
            </label>
            <input
          ...
          <p className="mb-4 text-sm text-muted-foreground">
            {t('gitPanel.newBranch.fromCurrent', { branch: currentBranch, defaultValue: 'This will create a new branch from the current branch ({{branch}})' })}
          </p>
            ...
            >
              {isCreatingBranch ? (
                ...
                <>
                  ...
                  <span>{t('gitPanel.newBranch.submit', 'Create Branch')}</span>
                </>
              )}
            </button>
          </div>
