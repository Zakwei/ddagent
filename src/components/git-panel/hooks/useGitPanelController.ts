...
export function useGitPanelController({
  ...
  const storedProvider = useSelectedProvider();

  // Commit messages should run on the provider this project actually uses —
  // the provider of its most recently active session. The global
  // `selected-provider` key only survives as a fallback for projects that have
  // no sessions yet.
  const sessionProvider = useMemo((): LLMProvider | null => {
    let latest: { provider: LLMProvider; at: number } | null = null;
    for (const session of selectedProject ? getProjectSessions(selectedProject) : []) {
      const provider = session.__provider ?? session.provider;
      if (!provider) {
        continue;
      }
      const at = Date.parse(
        session.lastActivity ?? session.createdAt ?? session.created_at ?? '',
      );
      const timestamp = Number.isFinite(at) ? at : 0;
      if (!latest || timestamp > latest.at) {
        latest = { provider, at: timestamp };
      }
    }
  ...
  const stageHunks = useCallback(
    async (filePath: string, hunkIndices: number[]) => {
      if (!selectedProject || !filePath || hunkIndices.length === 0) {
        return false;
      }

      try {
        const response = await fetchWithAuth('/api/git/stage-hunks', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            project: selectedProject.projectId,
  ...
  const unstageHunks = useCallback(
    async (filePath: string, hunkIndices: number[]) => {
      ...
      try {
        const response = await fetchWithAuth('/api/git/unstage-hunks', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            project: selectedProject.projectId,
