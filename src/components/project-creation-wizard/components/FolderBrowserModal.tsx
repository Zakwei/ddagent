...
export default function FolderBrowserModal({
  ...
  // Monotonic request id: only the latest browse may touch state, so a slow
  // earlier response can't overwrite a newer path/listing with stale data.
  ...
  const loadFolders = useCallback(async (pathToLoad: string) => {
    ...
    try {
      ...
      if (requestId !== browseSeqRef.current) return;
      ...
      if (requestId === browseSeqRef.current) {
  ...
  return (
    ...
    <Dialog open={isOpen} onOpenChange={(nextOpen) => { if (!nextOpen) handleClose(); }}>
      <DialogContent
        ...
        className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-lg border-gray-200 bg-white p-0 shadow-xl dark:border-gray-700 dark:bg-gray-800"
        onEscapeKeyDown={(event) => {
          // The new-folder input owns Escape first: it cancels folder creation
          // instead of closing the whole browser.
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <DialogTitle className="not-sr-only text-lg font-semibold text-gray-900 dark:text-white">Select Folder</DialogTitle>
          </div>
