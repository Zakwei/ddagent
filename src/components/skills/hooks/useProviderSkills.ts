...
): Promise<ProviderSkillRemoveResponse> => {
  const response = await authenticatedFetch(
    `/api/providers/${provider}/skills/${encodeURIComponent(directoryName)}`,
    { method: 'DELETE' },
  );
  const data = await toResponseJson<ApiResponse<ProviderSkillRemoveResponse>>(response);
  if (!response.ok || !data.success) {
...
export function useProviderSkills({ selectedProvider, currentProjects }: UseProviderSkillsArgs) {
  ...
  const deleteSkill = useCallback(async (directoryName: string) => {
    setDeleteError(null);
    try {
      const result = await deleteProviderSkill(selectedProvider, directoryName);
      clearProviderSkillCache(selectedProvider);
      await refreshSkills({ force: true });
      return result;
    } catch (error) {
