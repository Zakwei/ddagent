...
export function TaskMasterProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshTasks = useCallback(async () => {
    ...
    try {
      ...
      // A response that resolves after the user switched projects (or after a
      // newer request started) must not overwrite the current project's tasks.
      if (
        requestSequence !== tasksRequestSeqRef.current
        || currentProjectIdRef.current !== projectId
      ) {
        return;
      }
      ...
      if (requestSequence === tasksRequestSeqRef.current) {
