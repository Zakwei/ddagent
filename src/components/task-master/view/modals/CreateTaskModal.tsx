  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload.message ?? t('createTask.error', 'Failed to add task'));
      }
      ...
      setError(caughtError instanceof Error ? caughtError.message : t('createTask.error', 'Failed to add task'));
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(nextOpen) => { if (!nextOpen) handleClose(); }}>
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-700">
          <DialogTitle className="not-sr-only text-lg font-semibold text-gray-900 dark:text-white">{t('createTask.title', 'Add Task')}</DialogTitle>
          <button
            onClick={handleClose}
            aria-label={t('taskDetail.close', 'Close')}
        ...
        <div className="space-y-4 p-6">
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('createTask.titleLabel', 'Title')}</label>
            <input
              type="text"
              value={title}
              autoFocus
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('createTask.descriptionLabel', 'Description')}</label>
            <textarea
              rows={3}
              value={description}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('createTask.priorityLabel', 'Priority')}</label>
            <select
              value={priority}
          ...
          <div className="flex justify-end gap-2 border-t border-gray-200 pt-4 dark:border-gray-700">
            ...
            >
              {isSaving ? t('createTask.submitting', 'Adding...') : t('createTask.submit', 'Add Task')}
            </button>
          </div>
        </div>
      </DialogContent>
