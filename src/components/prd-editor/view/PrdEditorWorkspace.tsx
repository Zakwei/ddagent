...
export default function PrdEditorWorkspace({
  ...
  const handleOpenGenerateTasks = () => {
    if (!content.trim()) {
      setActionError('Please add content to the PRD before generating tasks.');
      return;
    }
    ...
    >
      ...
      >
        ...
        {(saveError || actionError) && (
          <div role="alert" className="border-b border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-800 dark:bg-red-900/20 dark:text-red-200">
            {saveError || actionError}
          </div>
        )}
