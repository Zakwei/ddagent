...
const readHandlePosition = (): number => {
  ...
  try {
    saved = localStorage.getItem(HANDLE_POSITION_STORAGE_KEY);
  } catch {
    // localStorage can throw (e.g. SecurityError when storage is blocked) —
    // fall back to the default instead of crashing the app shell.
    return DEFAULT_HANDLE_POSITION;
  }
...
// After a touchend, browsers synthesize mousedown/mouseup/click. Mouse events
// arriving within this window of a touch interaction must be ignored, or the
// synthetic mousedown would re-arm the drag state and the trailing click would
// toggle the panel after every touch drag.
...
export function useQuickSettingsDrag({ isMobile }: UseQuickSettingsDragProps) {
  ...
  const startDrag = useCallback((event: StartDragEvent) => {
    ...
    } else if (Date.now() - lastTouchTimeRef.current < SYNTHETIC_MOUSE_GRACE_MS) {
      return;
    }
