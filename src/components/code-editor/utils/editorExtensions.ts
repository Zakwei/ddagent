...
export const createScrollToLineExtension = ({
  file,
}: {
  file: CodeEditorFile;
}) => {
  if (!file.line || file.line < 1) {
    return [];
  }
  ...
  return [
    ViewPlugin.fromClass(class {
      constructor(view: EditorView) {
        // Defer until the document is fully mounted.
        setTimeout(() => {
          if (file.line && file.line <= view.state.doc.lines) {
            const line = view.state.doc.line(file.line);
            view.dispatch({
