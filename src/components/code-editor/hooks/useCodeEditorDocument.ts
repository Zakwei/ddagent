...
export const useCodeEditorDocument = ({ file, projectPath }: UseCodeEditorDocumentParams) => {
  ...
  // Bumped by retryLoad() to re-run the read effect after a failed load.
  ...
  useEffect(() => {
    // Guards against a slow read of the previous file resolving after the
    // user opened another one — without it the stale response would overwrite
    // `content`, and a later Ctrl+S would write that text into the new file.
    let cancelled = false;
    ...
    const loadFileContent = async () => {
      try {
        ...
        // Never stuff the error into `content`: the buffer mirrors the file on
        // disk, and saving placeholder text would corrupt it. Surface the
        // failure via `loadError` and keep the buffer empty so a stray save
  ...
  }, [file.diffInfo, file.name, fileDiffNewString, fileDiffOldString, fileName, filePath, fileProjectId, loadAttempt]);
  ...
  const handleSave = useCallback(async () => {
    ...
    // A missing/failed load leaves the buffer empty or stale — persisting it
    // would overwrite the real file with garbage. Refuse until content has
    // actually loaded.
