  ...
  useEffect(() => {
    ...
    const loadMedia = async () => {
      try {
        ...
        // A stale effect must not touch state: after cleanup ran, the new
        // effect already set loading=true, and clearing it here would flash
        // the "unable to display" error while the new fetch is in flight.
        if (!controller.signal.aborted) {
          setLoading(false);
        }
      }
