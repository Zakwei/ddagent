...
import { Check, Code2, Copy, Download, Eye, Maximize2, Minimize2, Pin, PinOff, Save, Settings as SettingsIcon, X } from 'lucide-react';

import { copyTextToClipboard } from '../../../../utils/clipboard';
import { usePinnedFiles } from '../../../chat/hooks/usePinnedFiles';
...
type CodeEditorHeaderProps = {
  ...
  saving: boolean;
  saveSuccess: boolean;
  /** Extra block (e.g. the file failed to load) on top of the in-flight save. */
...
export default function CodeEditorHeader({
  ...
  const { isPinned, pinFile, unpinFile } = usePinnedFiles(file.projectId);
  const filePinned = isPinned(file.path);
      ...
      <div className="flex min-w-0 flex-1 shrink items-center gap-2">
        <div className="min-w-0 shrink">
          ...
          <div className="flex min-w-0 items-center gap-1">
            ...
            <button
              type="button"
              onClick={() => (filePinned ? unpinFile(file.path) : pinFile(file.path))}
              className={`flex shrink-0 items-center justify-center rounded p-0.5 transition-colors ${
                filePinned
                  ? 'text-blue-600 dark:text-blue-400'
                  : 'text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-200'
              }`}
              title={filePinned ? labels.unpinFile : labels.pinFile}
              aria-label={filePinned ? labels.unpinFile : labels.pinFile}
              aria-pressed={filePinned}
            >
              {filePinned ? <PinOff className="h-3 w-3" /> : <Pin className="h-3 w-3" />}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        ...
        {isSidebar && onToggleExpand && (
          <button
            ...
            className="flex items-center justify-center rounded-md p-1.5 text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
            title={isExpanded ? (labels.collapse || 'Collapse to split') : (labels.expand || 'Expand editor')}
            aria-label={isExpanded ? (labels.collapse || 'Collapse to split') : (labels.expand || 'Expand editor')}
          >
