...
import { createMinimapExtension, createScrollToFirstChunkExtension, createScrollToLineExtension, getLanguageExtensions } from '../utils/editorExtensions';
...
export default function CodeEditor({
  ...
  const scrollToLineExtension = useMemo(
    () => createScrollToLineExtension({ file }),
    [file],
  );
  ...
  // The keydown listener is document-level; the container ref scopes
  // Escape/Ctrl+S to this editor instance so shortcuts typed in the chat or a
  // sibling pane don't close the editor or save its file.
  ...
  return (
    <>
      ...
      <div ref={containerRef} className={outerContainerClassName}>
        <div className={innerContainerClassName}>
          <CodeEditorHeader
            ...
            labels={{
              ...
              pinFile: t('actions.pinFile', 'Pin file to context'),
              unpinFile: t('actions.unpinFile', 'Unpin file from context'),
              ...
              collapse: t('toolbar.collapse', { defaultValue: 'Collapse to split' }),
              expand: t('toolbar.expand', { defaultValue: 'Expand editor' }),
            }}
          />
          ...
          {loadError && (
            <div className="flex items-center justify-between gap-2 border-b border-red-200 bg-red-50 px-3 py-1.5 text-xs text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-300">
              <span className="min-w-0 truncate">{loadError}</span>
              <button
                type="button"
                onClick={retryLoad}
                className="shrink-0 rounded px-2 py-0.5 font-medium transition-colors hover:bg-red-100 dark:hover:bg-red-900/40"
              >
