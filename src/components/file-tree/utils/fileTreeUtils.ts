...
export function filterFileTreeByModified(items: FileTreeNode[], sinceMs: number): FileTreeNode[] {
  return items.reduce<FileTreeNode[]>((filteredItems, item) => {
    const modifiedMs = item.modified ? new Date(item.modified).getTime() : 0;
    const isRecent = modifiedMs >= sinceMs;
    const filteredChildren =
      item.type === 'directory' && item.children ? filterFileTreeByModified(item.children, sinceMs) : [];

    if (isRecent || filteredChildren.length > 0) {
      filteredItems.push({
        ...item,
        children: filteredChildren,
      });
