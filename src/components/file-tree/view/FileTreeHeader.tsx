...
import { CalendarClock, ChevronDown, Eye, FileText, FolderPlus, List, Loader2, RefreshCw, Search, TableProperties, Upload, X } from 'lucide-react';
...
type FileTreeHeaderProps = {
  ...
  onContentSearchModeChange?: (mode: boolean) => void;
  // Toolbar actions
  onNewFile?: () => void;
  onNewFolder?: () => void;
      ...
      <div className="flex items-center gap-1.5">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder={
              contentSearchMode
                ? t('fileTree.searchContentPlaceholder')
                : t('fileTree.searchPlaceholder')
            }
            value={searchQuery}
        ...
        {onToggleRecentOnly && (
          <Button
            ...
            title={
              recentOnly
                ? t('fileTree.showAllFiles', 'Show all files')
                : t('fileTree.showRecentOnly', 'Show files changed in the last 7 days')
            }
            aria-label={
              recentOnly
                ? t('fileTree.showAllFiles', 'Show all files')
        ...
        {onContentSearchModeChange && (
          <Button
            ...
            size="sm"
            className="h-7 w-7 p-0"
            onClick={() => onContentSearchModeChange(!contentSearchMode)}
            title={
              contentSearchMode
                ? t('fileTree.searchByName', 'Search by name')
                : t('fileTree.searchInFiles', 'Search in files')
            }
            aria-label={
              contentSearchMode
                ? t('fileTree.searchByName', 'Search by name')
            ...
            aria-pressed={contentSearchMode}
          >
            {contentSearchMode ? (
