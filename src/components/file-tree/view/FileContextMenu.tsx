...
export default function FileContextMenu({
  ...
  useEffect(() => {
    ...
    // Move focus into the menu on open. Without this, focus stays on the
    // tree item underneath and Enter/Space would re-trigger it (opening the
    // file) instead of activating a menu option.
    ...
    return () => {
      ...
      // When the focused menu item unmounts, focus falls back to <body> —
      // move it back to whatever opened the menu. If a follow-up action
      // (e.g. a dialog) already took focus, leave it alone.
      if (previouslyFocusedElement && (!document.activeElement || document.activeElement === document.body)) {
  ...
  useEffect(() => {
    ...
    const handleKeyboardMenuNavigation = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowDown') {
        ...
        // Focus trap: keep Tab cycling through the menu items instead of
        // letting focus escape to the file tree behind the menu.
        event.preventDefault();
        ...
        // Only activate items that live inside this menu — Enter/Space on the
        // focused tree item behind the menu must not click it.
        if (activeElement && menuRef.current?.contains(activeElement) && activeElement.getAttribute('role') === 'menuitem') {
          event.preventDefault();
          activeElement.click();
        }
      }
