    ...
    >
      {viewMode === 'detailed' ? (
        <>
          ...
          <div className="col-span-4 text-sm text-muted-foreground">{formatRelativeTime(item.modified)}</div>
        </>
      ) : viewMode === 'compact' ? (
        <>
          ...
          <div className="ml-2 flex flex-shrink-0 items-center gap-3 text-sm text-muted-foreground">
            {item.type === 'file' && <span className="tabular-nums">{formatFileSize(item.size)}</span>}
          </div>
        </>
      ) : (
        <>
