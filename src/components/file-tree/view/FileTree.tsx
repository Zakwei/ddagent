...
import { FILE_TREE_RECENT_ONLY_STORAGE_KEY } from '../constants/constants';
...
import { useFileTreeContentSearch } from '../hooks/useFileTreeContentSearch';
...
import FileTreeSearchResults from './FileTreeSearchResults';
...
export default function FileTree({ selectedProject, onFileOpen }: FileTreeProps) {
  ...
  const [recentOnly, setRecentOnly] = useState(() => {
    try {
      return localStorage.getItem(FILE_TREE_RECENT_ONLY_STORAGE_KEY) !== 'false';
    } catch {
      return true;
    }
  });
  const toggleRecentOnly = useCallback(() => {
    setRecentOnly((previous) => {
      const next = !previous;
      try {
        localStorage.setItem(FILE_TREE_RECENT_ONLY_STORAGE_KEY, String(next));
      } catch {
  ...
  } = useFileTreeContentSearch({
    selectedProject,
    query: contentSearchQuery,
    enabled: contentSearchMode,
  });
  ...
  return (
    ...
    >
      ...
      <FileTreeHeader
        ...
        searchQuery={contentSearchMode ? contentSearchQuery : searchQuery}
        onSearchQueryChange={contentSearchMode ? setContentSearchQuery : setSearchQuery}
        contentSearchMode={contentSearchMode}
        onContentSearchModeChange={setContentSearchMode}
      ...
      {!contentSearchMode && viewMode === 'detailed' && filteredFiles.length > 0 && <FileTreeDetailedColumns />}
      ...
      {toast && (
        <div
          role="status"
          className={cn(
            'fixed bottom-[calc(1rem_+_env(safe-area-inset-bottom))] right-[calc(1rem_+_env(safe-area-inset-right))] z-[9999] px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 animate-in slide-in-from-bottom-2',
            toast.type === 'success'
