...
import { AlertTriangle, CalendarClock, Folder, Search } from 'lucide-react';
  ...
  return (
    <>
      {error ? (
        ...
        />
      ) : filteredFiles.length === 0 && recentOnly ? (
        <FileTreeEmptyState
          icon={CalendarClock}
          title={t('fileTree.noRecentFiles', 'No files changed in the last 7 days')}
          description={t('fileTree.showAllFilesHint', 'Turn off the recent filter to see everything.')}
        />
      ) : (
        <FileTreeList
          items={filteredFiles}
