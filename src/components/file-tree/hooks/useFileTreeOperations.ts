...
export function useFileTreeOperations({
  ...
  const handleConfirmRename = useCallback(async () => {
    ...
    try {
      ...
      // An editor showing the renamed file must follow the new path —
      // otherwise a later save writes under the old name and recreates it.
      const parentDir = renamingItem.path.includes('/')
        ? renamingItem.path.slice(0, renamingItem.path.lastIndexOf('/'))
        : '';
      window.dispatchEvent(
        new CustomEvent<EditorFileRenamedEventDetail>(EDITOR_FILE_RENAMED_EVENT, {
          detail: {
            projectId: selectedProject.projectId,
            oldPath: renamingItem.path,
            newPath: parentDir ? `${parentDir}/${renameValue}` : renameValue,
          },
        }),
      );
      onRefresh();
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      // Close the deleted file (or files under a deleted directory) in any
      // open editor — the buffer can no longer be saved back anyway.
      window.dispatchEvent(
        new CustomEvent<EditorFileDeletedEventDetail>(EDITOR_FILE_DELETED_EVENT, {
          detail: { projectId: selectedProject.projectId, path: item.path, type: item.type },
        }),
      );
      onRefresh();
      handleCancelDelete();
  ...
  const handleCopyPath = useCallback(async (item: FileTreeNode) => {
    try {
      // Await the write so the success toast only fires once the path is
      // actually on the clipboard (previously it reported success on failure).
