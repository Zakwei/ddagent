...
import { FALLBACK_PERMISSION_MODES } from '../../../../../../chat/constants/permissionModes';
import type { CodexPermissionMode, ProviderPermissionMode } from '../../../../../types/types';
...
const MODE_CARD_STYLES: Record<ProviderPermissionMode, { card: string; title: string; description: string }> = {
  default: {
    card: 'border-border bg-accent',
    title: 'text-foreground',
    description: 'text-muted-foreground',
  },
  acceptEdits: {
    card: 'border-green-400 bg-green-50 dark:border-green-600 dark:bg-green-900/20',
    title: 'text-green-900 dark:text-green-100',
    description: 'text-green-700 dark:text-green-300',
  },
  bypassPermissions: {
    card: 'border-orange-400 bg-orange-50 dark:border-orange-600 dark:bg-orange-900/20',
...
function ProviderPermissionModeSettings({
  agent,
  permissionMode,
  onPermissionModeChange,
}: ProviderPermissionModeProps) {
  const { t } = useTranslation('settings');
  const modes = FALLBACK_PERMISSION_MODES[agent] as ProviderPermissionMode[];
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <Shield className="h-5 w-5 text-green-500" />
          <h3 className="text-lg font-medium text-foreground">{t('permissions.permissionMode.title')}</h3>
        </div>
        <p className="text-sm text-muted-foreground">
          {t('permissions.permissionMode.description', { provider: PROVIDER_DISPLAY_NAMES[agent] })}
        </p>

        {modes.map((mode) => {
          const styles = MODE_CARD_STYLES[mode];
          const isSelected = permissionMode === mode;
          return (
            <div
              key={mode}
              className={`cursor-pointer rounded-lg border p-4 transition-all ${isSelected ? styles.card : UNSELECTED_MODE_CARD}`}
              onClick={() => onPermissionModeChange(mode)}
            >
              <label className="flex cursor-pointer items-start gap-3">
                <input
                  type="radio"
                  name={`${agent}PermissionMode`}
                  checked={isSelected}
                ...
                <div>
                  <div className={`flex items-center gap-2 font-medium ${styles.title}`}>
                    {t(`permissions.permissionMode.modes.${mode}.title`)}
                    {mode === 'bypassPermissions' && <AlertTriangle className="h-4 w-4" />}
                  </div>
                  <div className={`text-sm ${styles.description}`}>
                    {t(`permissions.permissionMode.modes.${mode}.description`)}
                  </div>
                </div>
              </label>
            </div>
...
type PermissionsContentProps = ClaudePermissionsProps | CursorPermissionsProps | CodexPermissionsProps | ProviderPermissionModeProps;
...
export default function PermissionsContent(props: PermissionsContentProps) {
  ...
  return <ProviderPermissionModeSettings {...props} />;
}
